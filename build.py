...
def make_pdf(config, data):
    ...
    # import pdfkit
    output_dir = config.get('output_dir', 'build')
    pdf_build_dir = config.get('pdf_build_dir', 'pdf_build')
    build(data, config, pdf_build_dir)
    input_file = os.path.join(pdf_build_dir, 'index.html')
    output_file = os.path.join(output_dir, config.get('pdf_file', 'resume.pdf'))
    # pdfkit.from_file(input_file, output_file)
    html = HTML(input_file, base_url=pdf_build_dir)
...
def main():
    ...
    if output_format == 'pdf':
        config['is_pdf'] = resume_data['config']['is_pdf'] = True
